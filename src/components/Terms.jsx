...
// 이용약관 모달. 개인정보처리방침(PrivacyPolicy)과 동일한 privacy-* 모달 스타일을 공유한다.
export default function Terms({ className = '', label = '이용약관' }) {
    ...
    <>
      ...
      {open && (
        <div className="privacy-overlay" onClick={() => setOpen(false)}>
          <div className="privacy-modal" role="dialog" aria-modal="true" aria-label="이용약관" onClick={(e) => e.stopPropagation()}>
            <div className="privacy-head">
              <h4 className="privacy-title">이용약관</h4>
              <button type="button" className="privacy-close" aria-label="닫기" onClick={() => setOpen(false)}>×</button>
            </div>
            <div className="privacy-body">
              <h5>제1조 목적</h5>
              <p>본 이용약관은 포커스특공대(이하 "사이트")의 서비스의 이용조건과 운영에 관한 제반 사항 규정을 목적으로 합니다.</p>
              ...
              <h5>제9조 서비스 이용 시간</h5>
              <p>
                ① 서비스 이용 시간은 업무상 또는 기술상 특별한 지장이 없는 한 연중무휴 1일 24시간을 원칙으로 합니다. 단, 사이트는 시스템 정기점검, 증설 및 교체를 위해 사이트가 정한 날이나 시간에 서비스를 일시중단 할 수 있으며 예정된 작업으로 인한 서비스 일시 중단은 사이트의 홈페이지에 사전에 공지합니다.<br />
              ...
              <h5>제10조 서비스 이용 해지</h5>
              <p>
                ① 회원이 사이트와의 이용계약을 해지하고자 하는 경우에는 회원 본인이 온라인을 통하여 등록해지 신청을 하여야 합니다.<br />
                ② 해지 신청과 동시에 사이트가 제공하는 사이트 관련 프로그램이 회원 관리 화면에서 자동적으로 삭제됨으로 운영자는 더 이상 해지신청자의 정보를 볼 수 없습니다.
              </p>

              <h5>제11조 서비스 이용 제한</h5>
              ...
              <h5>제12조 게시물의 관리</h5>
              <p>
                ① 사이트의 게시물과 자료의 관리 및 운영의 책임은 운영자에게 있습니다. 운영자는 항상 불량 게시물 및 자료에 대하여 모니터링을 하여야 하며, 불량 게시물 및 자료를 발견하거나 신고를 받으면 해당 게시물 및 자료를 삭제하고 이를 등록한 회원에게 주의를 주어야 합니다. 한편, 이용회원이 올린 게시물에 대해서는 게시자 본인에게 책임이 있으니 회원 스스로 본 이용약관에서 위배되는 게시물은 게재해서는 안 됩니다.<br />
                ② 정보통신윤리위원회 등 공공기관의 시정요구가 있는 경우 운영자는 회원의 사전동의 없이 게시물을 삭제하거나 이동할 수 있습니다.<br />
